...
private[spark] class BlockManager(
          ...
          if (!level.useMemory) {
            ...
            if (!asValues) {
              ...
              if (level.deserialized) {
                ...
                valuesBuffer ++= values
                memoryStore.putValues(blockId, Left(valuesBuffer), level, true).data match {
                  case Left(values2) =>
                    return Some(values2)
                  case _ =>
  ...
  private def doPut(blockId: BlockId, data: Either[Either[ArrayBuffer[Any], Iterator[Any]], ByteBuffer],
    ...
    if (info != null)  {
      info.synchronized {
        ...
        if (level.useDisk && !diskStore.contains(blockId)) {
          ...
          data match {
            case Left(elements) =>
              diskStore.putValues(blockId, Left(elements), level, false)
            case Right(bytes) =>
